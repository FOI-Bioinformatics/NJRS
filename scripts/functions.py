...
## File description:
## Random collection of misc functions used in development
...
def importGFF(path,rSorted=False,suppress_verbose=False,parse_all_keys=False):
    # check if no input, then return empty
    if not path: return {}
    if not suppress_verbose:
        print('Import GFF features...\n\t'+path)
        print('\tI WAS MADE FOR REPEATMASKER, CORRECT ME OR BE WARY THAT VARIABLE NAMES MAY BE UNINTUITIVE!')
...
def importReadSeqsFasta(path,noSequence=False,selection=set(),raw_header=False):
    read_seqs = {}
    with open(path,'r') as f:
        #add reads
        rn=None
        ...
        for line in f:
            ...
            if not line[0] == '>':
                ...
                if rn:
                    rn_old = rn.strip('\n').split()[0][1:]
                    # check if we wanted raw header
                    if raw_header:   rn_old = rn.strip('\n')[1:]
...
def plotHist(inp_arr,file_dest='',title=None,cutoff=None,cap=None,threshold=None,raisee=None,nbins=100):
    from matplotlib import pyplot as plt
    filt = inp_arr
...
def plotHist_classified(inp_dict,file_dest='',title=None,hide_legend=None,cutoff=None,cap=None,threshold=None,raisee=None,nbins=100):
    
    fig = plt.figure(figsize=(11,11))
    for classi,vals in inp_dict.items():
        filt = vals
        if cutoff != None:          filt = filterArr( filt, cutoff, 'cutoff' )
        if cap != None:             filt = filterArr( filt, cap, 'cap' )
        if threshold!= None:        filt = filterArr( filt, threshold, 'threshold' )
        ...
        plt.hist(filt,bins=nbins,label=classi)
    
    title_str = ''
    if title != None:       title_str = title
    if not hide_legend:     plt.legend()
    plt.title(title_str+' || numVals='+str(len(filt)))
